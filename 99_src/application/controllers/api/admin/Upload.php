...
class Upload extends Base_Controller {
	...
	public function image() {
		...
		if ($this->input->method() != 'post') {
			echo json_encode(makeResultErrorNotAllowedHttpMethod('Not allowed http method. (Allow only POST method)'), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
			exit();
		}
		...
		if ($this->upload->do_upload('uploadFile')) {
			...
			$result_array = makeResultSuccess($retUploadPath);
		} else {
			$uploadErrorData = $this->upload->display_errors();
			$result_array = makeResult(-999, 'image update failed.', $uploadErrorData);
		}
